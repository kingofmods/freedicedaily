$(document).ready(function() {
	...
	function randomIntFromInterval( min,max )
	{
      ...
      var msg = ['Thank you mateee! your hack is working boyyy',
				 ...
				 'hahaha I love this generator in 2 sec I generated unlimited vbucks bro',
				 'Whats up mates?',
				 'I just tested and generated 5 000 Spins thanks',
				 ...
				 'I generated today 3000 Spins ',
				 'How much do you play it daily?',
				 'Who is playing this game?',
				 'can I shutdown game now??',
				 'Hello anybody want to send him Spins? I generated it a lot of :D',
				 'where I can contact you to donate? I love you !!',
				 'this is working i cant belive juhuuu',
				 'happy new year lol i am just kidding happy life with a lot of Spins generated',
				 ...
				 'Cool :D :) <3',
				 'its to protect from spamming, first try to use, i got no Survey request, but for second try i need to get Finish 1 Survey',
	...
	setInterval( function(){ runAI();  }, 4500 ); // Interval for chat msgs
	setInterval( function(){ runGM(); }, 11500 ); // Interval for generated msgs
	...
	function SendChatMessage() {
		...
		if( chat_input_msg == "" ) {
			...
			output_window.append( "<li><span class='name' style='color: black;'><i class='fas fa-user'></i> " + default_user + "</span>: <span class='message'>" +  chat_input_msg +"</span></li>" );
			$(".livechatListArea").stop().animate({ scrollTop: $( ".livechatListArea" )[ 0 ].scrollHeight }, 1000 );
			$('.livechatMsg').val( '' );
		}
		
		setTimeout( function(){ CheckChatMessage( chat_input_msg ); }, 500 );
    ...
    function runAI() {
		...
		setTimeout(function(){
			output_window.append( "<li><span class='name' style='color: " + colors[ c ] + ";'><i class='fas fa-user'></i> " + user[ i ] + "</span>: <span class='message'>" + msg[ i ] + "</span></li>" );
		...
		$( '.typing-username' ).html( "<i class='fas fa-user'></i> " + user[ i ] );
		
    }
	
    function runGM() { 
	
		random_number = ( parseInt( Math.floor((Math.random() * 1000) + 1000) / 1000, 10 ) +1 ) * 500; 
		...
		setTimeout(function(){
			...
			$(".livechatListArea").stop().animate({ scrollTop: $( ".livechatListArea" )[ 0 ].scrollHeight }, 5000 );
		}, 3000);
